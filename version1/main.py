...
# เชื่อมกับไฟล์ config 
from config import (
    DATASET_CONFIG, FLOOD_EXPERIMENTS, CROSS_EXPERIMENTS, 
    TRAINING_CONFIG, get_experiment_name, print_experiment_header, 
...
def plot_confusion_matrix(confusion_matrix, experiment_name):
    ...
    # แปลงเป็นอาร์เรย์ numpy เพื่อการจัดการที่ง่ายขึ้น
    cm = np.array(confusion_matrix)
    
    # สร้างสีของตาราง Confusion Matrix
...
def run_single_experiment(data, task, experiment_config, min_vals=None, max_vals=None):
    """
    Run a single experiment based on configuration
    """
    variable = experiment_config['params']['variable']
    values = experiment_config['params']['values']
    fixed_params = experiment_config['params']['fixed_params']
    
    print(f"\n--- {experiment_config['name']} Experiments ---")
    ...
    for value in values:
        ...
        params['n_epoch'] = TRAINING_CONFIG['n_epochs']
        params['n_folds'] = TRAINING_CONFIG['n_folds']
        params['task'] = task
        
        # เพิ่ม min_vals และ max_vals สำหรับ regression
        if task == 'regression':
            params['min_vals'] = min_vals
            params['max_vals'] = max_vals
        
        # สร้างชื่อ experiment
        experiment_name = get_experiment_name(task, variable, value)
        params['experiment_name'] = experiment_name
        
        # รัน experiment
        run_experiment_detailed(data, **params)
...
def comprehensive_experiments():
    ...
    flood_data = load_flood_data(DATASET_CONFIG['flood_data_file'])
    flood_data, flood_min, flood_max = normalize_dataset(flood_data)
    
    cross_data = load_cross_pat(DATASET_CONFIG['cross_data_file'])
    
    # ============ เรียกใช้การทดลอง Flood Dataset  ============
    print_experiment_header("FLOOD DATASET EXPERIMENTS")
    
    for experiment in FLOOD_EXPERIMENTS:
    ...
    # ============ เรียกใช้การทดลอง Cross Pattern Dataset  ============
    print_experiment_header("CROSS PATTERN DATASET EXPERIMENTS")
